...
class FeatureAggregation(nn.Module):
    def __init__(self, module_dim=512):
        ...
        self.v_1 = nn.Linear(module_dim*2, module_dim, bias=False)
    ...
    def forward(self, question_rep, visual_feat):
        ...
        visual_feat = self.v_1(visual_feat)
...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, module_dim, word_dim, k_max_frame_level, k_max_clip_level, spl_resolution, vocab, question_type):
        ...
        self.swin_config = 'swim/configs/recognition/swin/swin_base_patch244_window1677_sthv2.py'

        self.swin_cfg = Config.fromfile(self.swin_config)
        self.swin_model = build_model(self.swin_cfg.model, train_cfg=None, test_cfg=self.swin_cfg.get('test_cfg'))
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, question,
        ...
        Args:
            ...
            question: [Tensor] (batch_size, max_question_length)
            question_len: [Tensor] (batch_size)
            raw_video: [Tensor] (batch_size, num_frames, channel, H, W)
        return:
            logits.
        """
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            swin_video_feat = self.swin_model.backbone(raw_video.permute(0, 2, 1, 3, 4)) # [batch_size, hidden, num_frames, height, width]
            visual_embedding = swin_video_feat.mean(dim=[3,4]).permute(0,2,1) # .permute(0,2,1) # [batch_size, num_frames/2, hidden]
            
            question_embedding = self.linguistic_input_unit(question, question_len.cpu())

            # swin_appearance_feat = swin_video_feat.view(batch_size, 4, 4, -1)
            # swin_motion_feat = swin_appearance_feat.clone().detach().requires_grad_(True).mean(2)
            # visual_embedding = self.visual_input_unit(swin_appearance_feat, swin_motion_feat, question_embedding)
    ...
    def load_swim_ckpt(self, ckpt_file):
        load_checkpoint(self.swin_model, ckpt_file, map_location='cpu', strict=True)